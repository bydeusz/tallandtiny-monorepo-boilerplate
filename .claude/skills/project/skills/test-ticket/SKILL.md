...
- **Run it:** from the worktree, start `pnpm dev` in the background (runs all apps on **hardcoded** ports — web 3000, api 3001, dashboard 3002, website 3003, prisma studio 5555, mailpit 8025) and wait until the app-under-test responds. Capture its **base URL** (the worktree's server). **Don't run two dev servers at once:** the ports are hardcoded, so a `pnpm dev` left running from another checkout both serves the wrong code and blocks this one (`EADDRINUSE`) — stop it first.
  - **`pnpm dev` also starts the mail worker — keep it whenever a criterion touches email.** `pnpm dev` runs `turbo run dev dev:worker`, and the api's **`dev:worker`** (a separate `main-worker.js` BullMQ process) is the **only** thing that actually sends mail — the api server just enqueues jobs to Redis. So if you start apps **selectively** to keep it light (the natural instinct for a single-ticket run — e.g. `pnpm --filter web dev` + `pnpm --filter api dev`), you MUST also start the worker: `turbo run dev dev:worker --filter=<app> --filter=api`. Skip it and every activation code, reset temp-password, and notification is silently **never sent**, even though the API still returns `200`/`201`. Prefer plain `pnpm dev` when email is in scope.
- **Test data & tokens — the database is a first-class source; don't fabricate data.** Prisma lives in **`@repo/database`**.
  ...
  - **Reading a token from the DB proves the flow can *continue* — it does NOT prove the *email was sent*.** Use `ActivationCode.code` from the DB to unblock a precondition, but when the criterion is about the mail itself ("activation code **in Mailpit**", "temporary password **emailed**"), the **Mailpit message is the proof**, not the DB row. And the reset temp-password has **no** DB fallback (hashed only), so that flow is untestable unless the worker is running and the mail actually lands.
- **Mail is enqueued, not sent inline — verify delivery in Mailpit for any email criterion.** A `2xx` from register/reset only means the job was **queued**. Prove it landed: `curl -s http://localhost:8025/api/v1/messages` (or the Mailpit UI at :8025) must show the message, and read the code/link **from that message**. **0 messages despite a `2xx` = the mail worker isn't running** (see "Run it" above) — that is a blocked environment, not a passing scenario. This is a scenario the writer/runner must include whenever email is in the flow.
...
- **Trusting a `2xx` (or a DB-read code) as proof an email was sent.** Mail is dispatched by a separate BullMQ **worker** that only `pnpm dev` (or `turbo run dev dev:worker`) starts; start apps selectively and you drop it, so register/reset return `200`/`201` but Mailpit stays empty. Verify the message in Mailpit; if it's 0, the worker is down — and a DB-read `ActivationCode` lets you *continue* the flow but does **not** satisfy an "email received" criterion (step 4).
